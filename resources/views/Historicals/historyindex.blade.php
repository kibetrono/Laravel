...
<form style="background-color:black;" action="/searchHistoricals" method="POST" role="search">
    {{ csrf_field() }}
...
<!---End ???????????---->
  ...
  {{ $historical->site_title }}</a><br>
            ...
            @if(Auth::user()->_id=="5dc4469e01a49c4c245bc932")                 
                    ...
                    <a style="margin:10px" href="{{url('editHistoricals', $historical->id)}}" class="btn btn-warning"><span class="fas fa-edit"> Edit</span></a>
                    </li>
                    <li role="presentation">
                    <form style="margin:10px" action="{{url('deleteHistoricals', $historical->id)}}" method="post">
                             @csrf
                    ...
                    <br><cite style="float:left">Posted on: {{date('M j, Y', strtotime($historical->updated_at))}} </cite>
                    @endif
		</ul>
        </div>
      @endforeach
