...
<form style="background-color:black;" action="/searchArts" method="POST" role="search">
    {{ csrf_field() }}
...
<h1  style="margin-top:5px;padding-top:2%;color:#fff;text-shadow: 0px 0px 4px #000;text-align:center">WEB GUIDE ARTS BOOKS</h1>
</div>
<!-- Quick links -->
<!-- Search -->
</div> <!-- /header -->
  ...
  @if(Session::has('success'))
      ...
      <div class="col-md-6">
		<ul style="margin-top:30px" class="list_group">
			<a href="{{$art->site_link}}"><img style='border-radius:20px;max-width:100px' src="{{ $art->filename}}" >
  {{ $art->site_title }}</a><br>
            {{ substr($art->site_description,0,200)}} ...
            <ul class="nav nav-pills">
            @if(Auth::user()->_id=="5dc4469e01a49c4c245bc932")                 
                    <li role="presentation">
                    <a style="margin:10px" href="{{url('editArts', $art->id)}}" class="btn btn-warning"><span class="fas fa-edit"> Edit</span></a>
                    </li>
                    <li role="presentation">
                    <form style="margin:10px" action="{{url('deleteArts', $art->id)}}" method="post">
                             @csrf
                    ...
                    <br><cite style="float:left">Posted on: {{date('M j, Y', strtotime($art->updated_at))}} </cite>
                    @endif
		</ul>
        </div>
      @endforeach
