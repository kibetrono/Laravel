...
<h1  style="margin-top:5px;padding-top:2%;color:#fff;text-shadow: 0px 0px 4px #000;text-align:center">WEB GUIDE NOVEL BOOKS</h1>
</div>
<!-- Quick links -->
<!-- Search -->
</div> <!-- /header -->
...
<div class="col-md-6">
		<ul style="margin-top:30px" class="list_group">
			<a href="{{$novel->site_link}}"><img style='border-radius:20px;max-width:100px' src="{{ $novel->filename}}" >
{{  $novel->site_title }}</a><br>
            {{ substr($novel->site_description,0,200)}} ...
            <ul class="nav nav-pills">
            @if(Auth::user()->_id=="5dc4469e01a49c4c245bc932")                 
                    <li role="presentation">
                    <a style="margin:10px" href="{{url('NovelEdit', $novel->id)}}" class="btn btn-warning"><span class="fas fa-edit"> Edit</span></a>
                    </li>
                    <li role="presentation">
                    <form style="margin:10px" action="{{url('deleteNovel', $novel->id)}}" method="post">
                             @csrf
                    ...
                    <br><cite style="float:left">Posted on: {{date('M j, Y', strtotime($novel->updated_at))}} </cite>
                    @endif
		</ul>
        </div>    
	@endforeach
